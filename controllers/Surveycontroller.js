...
export const addRecontact = async (req, res) => {
    try {
        const { parent_survey_id, project_name, description, loi, ir, sample_size, currency, start_date, end_date, cpi, notes, live_url, test_url } = req.body;
        if (!parent_survey_id) return res.status(400).json({ success: false, message: "parent_survey_id is required" });

        const parentSurvey = await Survey.getById(parent_survey_id);
        if (!parentSurvey) return res.status(404).json({ success: false, message: "Parent survey not found" });

        const survey_id = await Survey.generateSurveyId();
        await Survey.create({ survey_id, survey_type: "recontact", parent_survey_id, project_name, client_id: parentSurvey.client_id, project_manager_id: parentSurvey.project_manager_id, project_country: parentSurvey.project_country, description, loi, ir, sample_size, currency, start_date, end_date, cpi, notes, live_url, test_url, created_by: req.user?.id || null });

        await logActivity({ admin_id: req.user?.id, action: 'ADD', module: 'RecontactSurvey', description: `ReContact survey "${project_name}" created from parent ${parent_survey_id}`, ip_address: req.ip });

        return res.status(201).json({ success: true, message: "ReContact created successfully" });
    } catch (error) {
...
export const getSurveyRecontacts = async (req, res) => {
    try {
        const { id } = req.params;
        const data = await Survey.getRecontactsBySurvey(id);
        return res.status(200).json({ success: true, data });
    } catch (error) {
        return res.status(500).json({ success: false, error: error.message });
    }
...
export const addSurvey = async (req, res) => {
    try {
        ...
        const { project_name, client_id, project_manager_id, project_country, description, sales_manager_id, sales_project_id, loi, ir, sample_size, currency, start_date, end_date, link_type, term_point, comp_point, notes, cpi, live_url, test_url, status } = req.body;
...
export const updateSurvey = async (req, res) => {
    try {
        ...
        const updateData = { project_name, client_id, project_manager_id, project_country, description, sales_manager_id, sales_project_id, loi, ir, sample_size, currency, start_date, end_date, link_type, term_point, comp_point, notes, cpi, live_url, test_url, status };
        Object.keys(updateData).forEach(k => updateData[k] === undefined && delete updateData[k]);
        await Survey.update(id, updateData);

        await logActivity({ admin_id: req.user?.id, action: 'UPDATE', module: 'Survey', description: `Survey ID ${id} updated`, ip_address: req.ip });
...
export const deleteSurvey = async (req, res) => {
    try {
        ...
        await logActivity({ admin_id: req.user?.id, action: 'DELETE', module: 'Survey', description: `Survey ID ${id} deleted`, ip_address: req.ip });
...
export const getEligiblePartners = async (req, res) => {
    try {
        ...
        return res.status(200).json({ success: true, message: `${partners.length} eligible partners found`, filters_applied: { country: survey.project_country || null, sample_size: survey.sample_size || null, comp_point: survey.comp_point || null, term_point: survey.term_point || null }, data: partners });
    } catch (error) {
        return res.status(500).json({ success: false, message: "Server error!", error: error.message });
    }
};
...
export const assignPartners = async (req, res) => {
    try {
        ...
        if (!partner_ids || !Array.isArray(partner_ids)) return res.status(400).json({ success: false, message: "partner_ids array is required!" });
        ...
        await logActivity({ admin_id: req.user?.id, action: 'ASSIGN_PARTNERS', module: 'Survey', description: `${partner_ids.length} partner(s) assigned to survey ${survey.survey_id}`, ip_address: req.ip });

        return res.status(200).json({ success: true, message: `${partner_ids.length} partner(s) assigned successfully!` });
    } catch (error) {
        return res.status(500).json({ success: false, message: "Server error!", error: error.message });
    }
};
...
export const removePartner = async (req, res) => {
    try {
        ...
        await logActivity({ admin_id: req.user?.id, action: 'REMOVE_PARTNER', module: 'Survey', description: `Partner ID ${partnerId} removed from survey ${survey.survey_id}`, ip_address: req.ip });
...
export const updatePartnerAllocation = async (req, res) => {
    try {
        ...
        if (!allocated_size) return res.status(400).json({ success: false, message: "allocated_size is required!" });
        ...
        await logActivity({ admin_id: req.user?.id, action: 'UPDATE_ALLOCATION', module: 'Survey', description: `Partner ID ${partnerId} allocation updated to ${allocated_size} in survey ${survey.survey_id}`, ip_address: req.ip });
