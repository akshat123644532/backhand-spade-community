...
export const addSurveyGroupProject = async (req, res) => {
    try {
        ...
        if (!client_ids || client_ids.length === 0) return res.status(400).json({ success: false, message: "At least one client is required!" });

        const project_id = await SurveyGroupProject.create({ project_name, description, notes, status, created_by: req.user?.id || null });
        await SurveyGroupProject.addClients(project_id, client_ids);
        if (survey_ids && survey_ids.length > 0) await SurveyGroupProject.addSurveys(project_id, survey_ids);

        await logActivity({ admin_id: req.user?.id, action: 'ADD', module: 'GroupSurvey', description: `Group survey "${project_name}" added`, ip_address: req.ip });

        const fullProject = await SurveyGroupProject.getById(project_id);
        return res.status(201).json({ success: true, message: "Survey group project added successfully!", data: fullProject });
    } catch (error) {
...
export const updateSurveyGroupProject = async (req, res) => {
    try {
        ...
        if (client_ids && client_ids.length > 0) { await SurveyGroupProject.deleteClients(id); await SurveyGroupProject.addClients(id, client_ids); }
        if (survey_ids && survey_ids.length > 0) { await SurveyGroupProject.deleteSurveys(id); await SurveyGroupProject.addSurveys(id, survey_ids); }

        await logActivity({ admin_id: req.user?.id, action: 'UPDATE', module: 'GroupSurvey', description: `Group survey ID ${id} updated`, ip_address: req.ip });
...
export const toggleStatus = async (req, res) => {
    try {
        ...
        await logActivity({ admin_id: req.user?.id, action: 'STATUS_CHANGE', module: 'GroupSurvey', description: `Group survey ID ${id} status changed to ${status}`, ip_address: req.ip });
...
export const deleteSurveyGroupProject = async (req, res) => {
    try {
        ...
        await logActivity({ admin_id: req.user?.id, action: 'DELETE', module: 'GroupSurvey', description: `Group survey ID ${id} deleted`, ip_address: req.ip });
...
export const addSurveysToGroup = async (req, res) => {
    try {
        ...
        if (!survey_ids || !Array.isArray(survey_ids) || survey_ids.length === 0) return res.status(400).json({ success: false, message: "survey_ids array is required!" });
        ...
        await logActivity({ admin_id: req.user?.id, action: 'ADD_SURVEYS', module: 'GroupSurvey', description: `${survey_ids.length} survey(s) added to group ID ${id}`, ip_address: req.ip });
